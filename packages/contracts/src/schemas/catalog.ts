...
/**
 * Legal setback line — a red dashed non-build zone on the ground plane.
 * Points are board-% (same coordinate space as DesignSiteFrame.boundary).
 * Generated by the AI site-setup pipeline (survey + title ingestion) or
 * authored manually. Rendered as a THREE.LineDashedMaterial in the WebGL
 * studio (SetbackBoundaryLayer).
 */
export const SetbackLineSchema = z.object({
  id: z.string().uuid(),
  /** Ring or run of board-% points (min 2 for a line, 3+ for a closed ring). */
  points: z.array(DesignSiteFramePointSchema).min(2),
  /** Operator-facing label (e.g., "Front setback 3.0m", "Side setback 1.0m"). */
  label: z.string().max(80).optional(),
});
export type SetbackLine = z.infer<typeof SetbackLineSchema>;

/**
 * Building footprint — an uneditable 3D house mass that frames the negative
 * garden space. Points are board-% (same coordinate space as
 * DesignSiteFrame.boundary). Extruded to `height_m` in the WebGL studio
 * (SetbackBoundaryLayer) as a translucent dark-grey mass. Generated by the
 * AI site-setup pipeline or stamped from Vicmap/title data.
 */
export const BuildingFootprintSchema = z.object({
  id: z.string().uuid(),
  /** Closed ring of board-% points (min 3 for a polygon). */
  points: z.array(DesignSiteFramePointSchema).min(3),
  /** Massing height in metres (eaves / parapet, ground to top). */
  height_m: z.number().positive(),
  /** Operator-facing label (e.g., "Dwelling", "Garage"). */
  label: z.string().max(80).optional(),
});
...
export const DesignCanvasSchema = z.object({
   ...
   * Generated by the AI site-setup pipeline or authored manually.
   * Absent on legacy canvases = [].
   */
  setback_lines: z.array(SetbackLineSchema).default([]),
  /**
   * Building footprints — uneditable 3D house masses that frame the negative
   * garden space. Generated by the AI site-setup pipeline or stamped from
   * Vicmap/title data. Absent on legacy canvases = [].
   */
  building_footprints: z.array(BuildingFootprintSchema).default([]),
  /**
...
export const UpsertDesignCanvasSchema = z.object({
  ...
  /** Legal setback lines — see DesignCanvasSchema.setback_lines. */
  setback_lines: z.array(SetbackLineSchema).optional(),
  /** Building footprints — see DesignCanvasSchema.building_footprints. */
  building_footprints: z.array(BuildingFootprintSchema).optional(),
  artboard_ids: z
    .array(z.enum(["plan", "fit", "elev-N", "elev-E", "elev-S", "elev-W"]))
    .max(8)
    .optional(),
