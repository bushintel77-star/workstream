...
/* Same-origin proxy for site photo underlays (multipart aerial upload —
 * replaces the auto aerial for sketch tracing). Forwards auth to the API. */
export async function POST(
  req: NextRequest,
  ctx: { params: Promise<{ id: string }> },
) {
  const { id } = await ctx.params;
  const upstream = `${getApiUrl()}/projects/${id}/aerial/upload`;
