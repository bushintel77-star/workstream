...
/* Same-origin proxy for vision photo-measurement uploads (multipart photo +
 * optional hint) from the web measurements surface. Forwards auth to the API. */
export async function POST(
  req: NextRequest,
  ctx: { params: Promise<{ id: string }> },
) {
  const { id } = await ctx.params;
  const upstream = `${getApiUrl()}/projects/${id}/measurements/photo`;
