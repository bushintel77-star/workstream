...
describe("ToolFlyout flyout registration", () => {
  it("registers only draw + plant + mass tools (real state-backed content)", () => {
    ...
    // Tier-1 split: MASS carries the target-plane widget (activePlaneId is
    // real store state — the depth rail + layers panel read it), so a
    // flyout for mass is genuine content, not a dead panel.
    expect(FLYOUT_TOOLS.has("mass")).toBe(true);
  });

  it("does not register tools with no real parameter surface", () => {
    // GRADE/MEASURE tools have no genuine per-tool parameter state yet,
    // so a flyout must NOT appear (would be a dead control).
    for (const tool of ["contour", "slope", "cutfill", "path", "dim", "section"] as const) {
