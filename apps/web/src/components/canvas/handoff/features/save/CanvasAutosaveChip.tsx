...
/**
 * Compact canvas version / autosave chip — replaces the large green save toast.
 * Lives in the header; never a full-width banner on the drawing.
 */
export function CanvasAutosaveChip({
  status,
  savedTick,
  revision,
  errorKind = null,
  ...
  return (
    <button
      type="button"
      className={`${css.chip}${busy ? ` ${css.chipBusy}` : ""}${status === "saved" ? ` ${css.chipSaved}` : ""}`}
      data-testid="autosave-tick"
      data-status={status}
      aria-live="polite"
      aria-label={
