...
| 12 | Current status document restored and kept authoritative | User prompt | CLOSED; `docs/WORKSTREAM-STATUS.md` reflects current verification and remaining human-owned work | Doc | Agent | P2 |
| 13 | Human ops runbook with copy-paste steps for Clerk, Redis, Sentry, Fly scale, EAS, external keys, OpenTelemetry, Litestream, and branch protection | User prompt | CLOSED in this docs pass; duplicate/corrupt sections removed and API/web Clerk variable names corrected | Doc | Agent | P0 |
