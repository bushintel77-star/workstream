...
|---|------|--------|--------|----------|-------|----------|
| 1 | Clerk live application plus `CLERK_SECRET_KEY` on construct-api and `CLERK_SECRET_KEY` + `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` on construct-web | OUTSTANDING.md | NOT DONE | HumanOps | Human | P0 |
...
| 12 | Human ops runbook with copy-paste steps for Clerk, Redis, Sentry, Fly scale, EAS, external keys, branch protection, OpenTelemetry endpoint, Stripe, and Litestream | User prompt | CLOSED in `814a3cb` | Doc | Agent | P0 |
