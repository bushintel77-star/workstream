...
2. **Present tab (compose).** Woven into the *same product* as another tab
   across the top — beside Survey / Sketch / Cad / Elevation / Quote — not a
   separate app or a second page. Clicking Present opens a **fresh canvas for
   formatting**: AI dissects the plan into sections and feature areas, the
   designer shuffles those panels on a page, and AI applies editorial formatting
   to produce print-ready decks, quotations, mood boards and concept sketches.
...
## 2. Same product, one new tab (honours "not a second product")
...
The one narrow revision: SHEET-PRESENTATION said "not a separate app **or mode
tab**" and kept compose as an in-studio peel on the design canvas. Present adds a
sibling **tab** (a fresh formatting canvas), while the inline fit-sheet peel stays
as the quick in-studio lens for a single sheet. So the "one canvas" rule relaxes
to "one canvas *per tab*": the design tab keeps its plan-truth canvas untouched;
Present gets its own clean canvas for storytelling. Recorded here as a decision,
not drift. If accepted, SHEET-PRESENTATION.md gets a header note pointing here.
...
- `template_id` constrained to a **closed enum of 4–5 editorial templates** (not
  a free string), each defining its named slots (blurb, four drawing squares,
  labour + product schedule).
...
- Persistence: sits beside `presentation_pack` on the project, not inside
  `DesignCanvas` (separation of concerns — the plan does not own the deck).
...
- **Phase 0 — this brief + schema brief.** Ratify the workspace split and the
  `PresentationDocument` schema. No UI.
- **Phase 1 — Present view shell + manual composer.** New route/view, page +
  panel model, drag/resize/reorder, reuse image layers and typed widgets, PDF
  export. No AI yet. Proves the separation and the print path.
- **Phase 2 — AI plan dissection.** Auto-generate feature-area and aspect panels
  from the finished plan; designer accepts/re-crops (ghost-until-accept, matching
  the AI-CAD pattern).
...
1. **Entry:** *Resolved* — a top-level tab beside Survey / Sketch / Cad /
   Elevation / Quote, same product, opening a fresh formatting canvas. Still open:
   is the tab enabled always, or only once the design is marked complete?
2. **Live vs snapshot:** should plan-crop panels stay live (re-render if the plan
   changes) or freeze at the moment of composition? (Recommend live, with an
   explicit "freeze for issue" like a drawing revision.)
3. **House style:** one Curtis template family to start, or per-practice theming
   (the TRIAS reference implies white-label title blocks)?
