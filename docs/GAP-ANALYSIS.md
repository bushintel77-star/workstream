...
| CI: deploy api + web on `main` | same; web `--build-arg NEXT_PUBLIC_API_URL=…` |
...
- [ ] GitHub secret `BROKKER` or `FLY_API_TOKEN` — valid Fly deploy token
- [ ] Branch protection on `main` — require CI green (repo settings)
...
| API root in browser | 404 JSON — expected |

## 3. Tier-1 — 36 Wrights Terrace, Prahran
...
| --- | --- | --- |
| Pipeline **costing totals** vs workbook | Portal may show rate-card math ? $58,410.35 target | Seed Tier-1 scenario or override in cost job when `isTier1…` |
...
## 4. Quality & scale (P1–P3)

See `OUTSTANDING.md` for the full punch list. Highest leverage next:

1. Playwright — operator happy path (create project ? design ? quote link).
2. Contract tests — parse every API response with its Zod schema.
3. BullMQ + Redis — async pipeline jobs.
4. EAS / TestFlight — mobile distribution.
