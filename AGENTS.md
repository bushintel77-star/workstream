...
- **[`docs/GOLD-STANDARD-2026.md`](docs/GOLD-STANDARD-2026.md)** — SUPREME. The master architectural brief. "The drawing is the product." Zero-Chrome, WebGL primary surface, Paper Cards (opaque `--la-surface` panels since the 2026-08-25 LA amendment — resting frost/blur retired; `--gs-shadow-1..4` neutral shadow tiers), Studio Paper tokens. If a change contradicts this doc, this doc wins.
- **[`apps/web/src/styles/tokens.css`](apps/web/src/styles/tokens.css)** — the token source of truth since the 2026-09-04 design-system rebuild collapsed `--gs-*`/`--lc-*`/`--la-*` into ONE `--ws-*` namespace (the standalone tokens doc was purged with the legacy design-doc set). Studio Paper `#F4F4F4` canvas, stark-white active chrome (`--ws-active` `#E8EAEC` + `--ws-active-ink`), `#0030CF` Truth Anchor data stroke (`--ws-dwg-truth`), `#D2564F` conflict crimson (`--ws-conflict`) — conflict-only, never CTA. Drawing tokens live under `--ws-dwg-*`. Fonts: Space Grotesk (technical/numeric), Inter (UI). Raw `#hex` in chrome is CI-gated (`scripts/check-handoff-chrome-colors.mjs`); reference `--ws-*` tokens by name.
...
- **[`docs/AEC-2026-RESEARCH-ADOPTION.md`](docs/AEC-2026-RESEARCH-ADOPTION.md)** — decision record for the adopted AEC-2026 research scope (ResCode A2-6 canopy compliance threaded survey→quote, motion-aware chrome recede, ARIA graphics tree) plus the rejected items (dark mode, WebGPU-first, OCCT, Forma) with reasons and verified regulatory facts. Read before touching compliance, chrome recede, or canvas a11y.
